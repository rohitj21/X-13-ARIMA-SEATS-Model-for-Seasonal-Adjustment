...
c-----------------------------------------------------------------------
      IF(Kfulsm.eq.1)THEN
       CALL writln('WARNING: No seasonal adjustment has been done becaus
     &e type=summary',fhnote,Mt2,T)
       CALL writln('         occurs in the x11 spec.  Therefore no slidi
     &ng spans analysis',fhnote,Mt2,F)
       CALL writln('         has been done.',fhnote,Mt2,F)
       IF(Iagr.eq.2)THEN
        CALL writln('         The unadjusted series from this spec file
     &has been incorporated',fhnote,Mt2,T)
        CALL writln('         into the composite seasonal adjustment for
     & each of the sliding',fhnote,Mt2,F)
        CALL writln('         spans.',fhnote,Mt2,F)
       ELSE
...
c-----------------------------------------------------------------------
      IF(Lncset.and.Lnlset)THEN
       IF(Length.LT.(Nlen+(Ncol-1)*Ny))THEN
        ...
        Issap=0
        CALL writln('NOTE: Not enough data to produce '//
     &              numstr(1:(ipos-1))//' sliding spans of length '//
     &              numstr(ipos:(ipos2-1))//'.',fhnote,Mt2,T)
       END IF
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
       ...
       IF(.not.Lncset)THEN
        IF(Lnlset)THEN
         ...
         ELSE IF(ncmax.le.1)THEN
          Issap=0
          CALL writln('NOTE: Not enough data to perform sliding spans an
     &alysis.',fhnote,Mt2,T)
          ipos=1
          CALL itoc(Nlen,numstr,ipos)
          CALL writln('      '//
     &         'Must be able to form at least two spans of length '//
     &         numstr(1:ipos-1)//'.',fhnote,Mt2,F)
         ELSE
          Ncol=ncmax
         END IF
        ELSE
     ...
     &                fhnote,Mt2,T)
         ELSE
          ...
          IF(Ncol.le.1)THEN
           Issap=0
           CALL writln('NOTE: Not enough data to perform sliding spans a
     &nalysis.',fhnote,Mt2,T)
           CALL writln('      Must have at least 3 years of data in each
     & span to perform',fhnote,Mt2,F)
           CALL writln('      sliding spans analysis for stable seasonal
     & filters.',fhnote,Mt2,F)
          END IF
         END IF
...
c-----------------------------------------------------------------------
     ...
     &               numstr(ipos:(ipos2-1))//'.',fhnote,Mt2,T)
       ...
       ELSE
        ...
        ELSE IF(ncmax.lt.2)THEN
         Issap=0
         CALL writln('NOTE: Not enough data to perform sliding spans ana
     &lysis.',fhnote,Mt2,T)
         IF(Lnlset)THEN
          ipos=1
          CALL itoc(Nlen,numstr,ipos)
     ...
     &         numstr(1:ipos-1)//'.',fhnote,Mt2,F)
         ELSE
          CALL writln('      Must be able to form at least two sliding s
     &pans.',fhnote,Mt2,F)
         END IF
        ELSE
         Ncol=ncmax
...
c-----------------------------------------------------------------------
      ...
      ELSE 
       ...
       IF(i.ge.Ny)THEN
        ...
        Ic=(Icyr-Iyr)*Ny+Icm
       ELSE
        CALL writln('WARNING: Date of the first sliding spans comparison
     & is set too early.',fhnote,Mt2,T)
        CALL writln('         This date will be reset so that it is one 
     &year after the',fhnote,Mt2,F)
        CALL writln('         starting date of the first span.',fhnote,
     &              Mt2,F)
        Icm=Im
        Icyr=Iyr+1
        Ic=Im+Ny
